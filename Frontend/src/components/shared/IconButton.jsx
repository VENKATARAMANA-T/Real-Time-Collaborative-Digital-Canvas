...
const IconButton = ({ icon: Icon, active, onClick, label, disabled, title, description, tooltipAlign = 'center' }) => {
  ...
  return (
    ...
    >
      ...
      {(title || label) && (
        <div className={`absolute top-[calc(100%+8px)] z-[300] pointer-events-none
          opacity-0 group-hover:opacity-100 scale-95 group-hover:scale-100
          transition-all duration-200 origin-top ${hAlign}`}>
          {/* Arrow pointing UP */}
          <div className={`absolute -top-[5px] w-2.5 h-2.5 bg-[#0f1924] border-l border-t border-[#2d3a4b] rotate-45 ${arrowAlign}`} />
          <div className="bg-[#0f1924] border border-[#2d3a4b] rounded-xl shadow-2xl px-3 py-2 min-w-[120px] w-max max-w-[250px]">
            <p className="text-white text-[11.5px] font-semibold leading-tight">{title || label}</p>
            {description && <p className="text-zinc-400 text-[10.5px] leading-snug mt-0.5 whitespace-normal break-words">{description}</p>}
          </div>
        </div>
      )}
    </button>
