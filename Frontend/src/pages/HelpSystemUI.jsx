...
export default function HelpSystemUI() {
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ease-in-out ${isDarkMode ? 'bg-gray-900 text-white' : mode === 'advanced' ? 'bg-gradient-to-br from-purple-50 via-violet-50 to-indigo-50 text-gray-900' : 'bg-gradient-to-br from-indigo-50 via-white to-purple-50 text-gray-900'}`}>
          ...
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center space-x-4">
              <button
                ...
                className="p-2 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-lg transition-colors duration-200 mr-2"
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {mode === 'advanced' && !loading && (
          <div className={`mb-6 px-4 py-3 rounded-xl border flex items-center justify-between animate-fade-in ${isDarkMode ? 'border-purple-500/30 bg-purple-900/10' : 'border-purple-300 bg-gradient-to-r from-purple-50 to-violet-50'}`}>
            <div className="flex items-center gap-3">
              <div className={`p-2 rounded-lg ${isDarkMode ? 'bg-purple-800/40' : 'bg-purple-100'}`}>
                <Zap className={`w-4 h-4 ${isDarkMode ? 'text-purple-400' : 'text-purple-600'}`} />
              </div>
              <div>
                <p className={`text-sm font-semibold ${isDarkMode ? 'text-purple-300' : 'text-purple-700'}`}>Advanced Mode Active</p>
                <p className={`text-xs ${isDarkMode ? 'text-purple-400/70' : 'text-purple-500'}`}>Compact layout · Keyboard hints · Full article access</p>
              </div>
            </div>
            <kbd className={`hidden sm:inline-flex px-2 py-1 text-xs rounded border font-mono ${isDarkMode ? 'border-purple-600/30 text-purple-400 bg-purple-900/20' : 'border-purple-300 text-purple-600 bg-white'}`}>Ctrl+M to switch</kbd>
          </div>
        )}
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between mb-6">
            <h2 className={`text-2xl font-bold ${mode === 'advanced' ? (isDarkMode ? 'text-purple-300' : 'text-purple-700') : ''}`}>Help Articles</h2>
            {mode === 'advanced' && ( <span className={`text-xs font-mono px-3 py-1 rounded-full border ${isDarkMode ? 'bg-purple-900/20 text-purple-400 border-purple-700/40' : 'bg-purple-50 text-purple-600 border-purple-200'}`}>⚡ Advanced View</span> )}
          </div>
            ...
            {loading ? ( <>{[1, 2, 3, 4, 5, 6].map(i => <SkeletonCard key={i} />)}</> ) : filteredArticles.length > 0 ? (
              filteredArticles.map((article) => {
                if (mode === 'advanced') {
                   ...
                   return (
                      <div key={article.id} onClick={() => setSelectedArticleId(article.id)} className={`group flex items-center justify-between p-4 rounded-lg border-l-4 border-l-purple-500 border ${isDarkMode ? 'bg-gray-800 border-gray-700 hover:bg-gray-750' : 'bg-white border-gray-200 hover:shadow-md'} transition-all duration-200 cursor-pointer`}>
                         <div className="flex items-center space-x-4">
                            <div className={`p-2 rounded-md ${isDarkMode ? 'bg-purple-900/30' : 'bg-purple-50'}`}><article.icon className={`w-5 h-5 ${isDarkMode ? 'text-purple-400' : 'text-purple-600'}`} /></div>
        ...
        {mode === 'advanced' && !loading && (
          <div className="mb-8 animate-fade-in">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className={`text-lg font-bold ${isDarkMode ? 'text-purple-300' : 'text-purple-700'}`}>Quick Reference</h2>
              <span className={`text-xs px-2 py-0.5 rounded-full font-mono ${isDarkMode ? 'bg-purple-900/30 text-purple-400' : 'bg-purple-100 text-purple-600'}`}>Shortcuts</span>
            </div>
            <div className={`grid grid-cols-2 md:grid-cols-3 gap-2 p-4 rounded-xl border ${isDarkMode ? 'bg-gray-800/60 border-purple-900/30' : 'bg-purple-50 border-purple-200'}`}>
              {[
                ...
                { keys: 'F', action: 'Pencil' },
                { keys: 'E', action: 'Eraser' },
                { keys: 'T', action: 'Text Tool' },
              ].map(({ keys, action }) => (
                <div key={keys} className={`flex items-center justify-between px-3 py-2 rounded-lg text-sm ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-purple-100'} border`}>
                  <span className={`${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>{action}</span>
                  <kbd className={`ml-2 px-2 py-0.5 text-xs rounded font-mono border ${isDarkMode ? 'bg-purple-900/30 text-purple-300 border-purple-700/50' : 'bg-purple-100 text-purple-700 border-purple-200'}`}>{keys}</kbd>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="mb-8">
          <h2 className={`text-2xl font-bold mb-6 ${mode === 'advanced' ? (isDarkMode ? 'text-purple-300' : 'text-purple-700') : ''}`}>Frequently Asked Questions</h2>
          <div className="space-y-4">
            {filteredFaqs.length > 0 ? (
              filteredFaqs.map((faq, idx) => (
                mode === 'advanced' ? (
                  <div key={idx} className={`rounded-lg border-l-4 border-l-purple-500 border transition-colors duration-300 ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                    <button onClick={() => setExpandedFaq(expandedFaq === idx ? null : idx)} className="w-full px-4 py-3 flex items-center justify-between">
                      <span className={`font-medium text-sm text-left ${isDarkMode ? 'text-purple-300' : 'text-purple-700'}`}>{faq.q}</span>
                      <ChevronDown className={`w-4 h-4 flex-shrink-0 ml-2 transition-transform duration-300 ${isDarkMode ? 'text-purple-400' : 'text-purple-600'} ${expandedFaq === idx ? 'rotate-180' : ''}`} />
                    </button>
                    {expandedFaq === idx && (<div className={`px-4 pb-3 text-sm transition-colors duration-300 ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>{faq.a}</div>)}
                  </div>
                ) : (
      ...
      <style>{`@keyframes twinkle { 0%, 100% { opacity: 1; transform: scale(1); } 50% { opacity: 0.3; transform: scale(0.8); } } .animate-twinkle { animation: twinkle 2s ease-in-out infinite; } .animate-fade-in { animation: fadeIn 0.3s ease-out; } @keyframes fadeIn { from { opacity: 0; transform: translateY(10px); } to { opacity: 1; transform: translateY(0); } }`}</style>
    </div>
  );
}
