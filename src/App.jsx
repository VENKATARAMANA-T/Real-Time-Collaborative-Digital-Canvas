...
export default function App() {
  ...
  useEffect(() => {
    const handleScroll = (e) => {
      if (showDashboard || showAuth) return; // Disable scroll if modal/dashboard open
      const now = Date.now();
          ...
          <div className="hidden md:flex items-center gap-8">
            {['Home', 'Features', 'Pricing'].map((item, idx) => (
              <button 
                ...
                className={`text-sm font-medium transition-colors hover:drop-shadow-[0_0_10px_rgba(255,255,255,0.5)] ${currentSlide === idx && !showDashboard ? 'text-white' : 'text-slate-400 hover:text-white'}`}
              >
                {item}
              </button>
            ))}
        ...
        <div className={`slide flex items-center ${currentSlide === 0 ? 'active' : currentSlide > 0 ? 'prev' : ''}`}>
          <div className="container mx-auto px-6 grid lg:grid-cols-2 gap-12 items-center">
            ...
            <div className="relative animate-scale-in" style={{ animationDelay: '0.5s' }}>
              ...
              <div className="glass-card rounded-2xl p-2 relative overflow-hidden h-[500px]">
